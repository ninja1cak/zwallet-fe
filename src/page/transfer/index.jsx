...
function Transfer () {
    ...
    const getUser = async () => {
        try {
            const {data} = await api.get(`/user/all?limit=5&page=${currentPage}`)
            console.log(data)
            setUser(data.data)
    ...
    return(
        ...
        <div className="flex flex-row w-4/5 mx-auto">
            <div></div>
            <div className="flex flex-col w-full bg-slate-50 rounded-lg px-5 py-10">
                ...
                <input type="text"   placeholder="Search Receiver Here" className="input border-none w-full bg-gray-100 focus:outline-none" />
                </div>
